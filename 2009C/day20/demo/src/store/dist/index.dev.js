...
var _default = new _vuex["default"].Store({
  ...
  mutations: {
    ...
    // 添加/编辑 地址
    setAddress: function setAddress(state, form) {
      var index = state.addressList.findIndex(function (item) {
        return item.id == form.id;
      });

      if (form.isDefault) {
        // 设置为默认地址 要把其他的isDefault变成false
        state.addressList.forEach(function (item) {
          item.isDefault = false;
        });

        if (index > -1) {
          // 编辑地址 因为设置默认地址 先删掉原来的地址
          state.addressList.splice(index, 1); // 将编辑后的结果追加到数组头部

          return state.addressList.unshift(_objectSpread({}, form, {
            address: form.addressDetail
          }));
        } // 新增地址


        state.addressList.unshift(_objectSpread({}, form, {
          address: form.addressDetail
        }));
        return;
      } // 没设置为默认地址
      ...
      if (index > -1) {
        return state.addressList.splice(index, 1, _objectSpread({}, form, {
          address: form.addressDetail
        }));
      } // 没设置为默认地址 也不是编辑
    ...
    // 新增历史记录
    addHistory: function addHistory(state, value) {
