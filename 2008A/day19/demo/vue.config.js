  ...
  devServer:{
    onBeforeSetupMiddleware({app}){
      ...
      app.get('/api/search',(req,res)=>{
        // tabIndex 下标  value 搜索内容
        let { tabIndex, value } = req.query
        if(tabIndex == 0){
          // total一共有多少条数据
          return res.send(data.tab1.filter(item => (item.title.includes(`${value}`)||item.contnet.includes(`${value}`))))
        }
        if(tabIndex == 1){
          return res.send(data.tab2.filter(item => (item.title.includes(`${value}`)||item.contnet.includes(`${value}`))))
        }
        if(tabIndex == 2){
          return res.send(data.tab3.filter(item => (item.title.includes(`${value}`)||item.contnet.includes(`${value}`))))
        }
        if(tabIndex == 3){
          return res.send(data.tab4.filter(item => (item.title.includes(`${value}`)||item.contnet.includes(`${value}`))))
        }
      })
    }
  }
